...
pub async fn create_signing_key(ttl_minutes: u32, auth_token: String, endpoint: Option<String>) -> Result<(), CliError> {
...
pub async fn revoke_signing_key(key_id: String, auth_token: String, endpoint: Option<String>) -> Result<(), CliError> {
    let mut client = get_momento_client(auth_token, endpoint).await?;
...
pub async fn list_signing_keys(auth_token: String, endpoint: Option<String>) -> Result<(), CliError> {
    let mut client = get_momento_client(auth_token, endpoint).await?;
